    ...
    <div class="main-content" :class="{ 'main-content-mobile': isMobile }">
      <div class="add-user-container">
        <div class="kinetic-card">
          ...
          <form @submit.prevent="handleSubmit" class="add-user-form" v-if="!loading">
            <div class="form-grid">
              ...
              <div class="form-group" style="grid-column: span 2;">
                ...
                <div class="input-wrapper-kinetic" :class="{ 'warning-border': form.roleName !== originalRole.roleName }">
                ...
                <p v-if="form.roleName !== originalRole.roleName" style="font-size: 0.75rem; color: var(--admin-accent); font-weight: 700; margin-top: 4px;">Cambio detectado: {{ originalRole.roleName }} → {{ form.roleName }}</p>
              </div>
            </div>

            <div class="form-actions">
              <button type="submit" class="submit-btn" :disabled="submitting || form.roleName === originalRole.roleName">
                <ion-icon :icon="submitting ? refreshOutline : checkmarkOutline" :class="{ 'spin-kinetic': submitting }" style="margin-right: 10px;"></ion-icon>
                {{ submitting ? 'Guardando...' : 'Actualizar Rol Kinetic' }}
              </button>
            </div>
          </form>
          ...
          <KineticLoading 
            v-else
            title="Sincronización de Rol"
            message="Obteniendo definición de permisos desde el núcleo Kinetic..."
          />
        </div>
...
const originalRole = reactive({ roleName: '' })
...
const handleSubmit = async () => {
  if (form.roleName.trim() === originalRole.roleName) return
  try {
    submitting.value = true
    const success = await userService.updateRole(parseInt(roleId), { roleName: form.roleName.trim() })
    if (success) {
